...
class CartService {
    async getByCustomerId(currentUserId) {
        let existingCart = await _repository.cartRepository.getCartByCustomerId(currentUserId);

        if (!existingCart) {
            throw Object.assign(new Error("Không tìm thấy giỏ hàng của user này"), { status: 404 });
        }

        let updatedItems = await Promise.all(existingCart.items.map(async (item) => {
            let cosmetic = await _repository.comesticRepository.getById(item.comestic_id);
            return {
                _id: item.id,
    ...
    async add(currentUserId, cartRequest) {
        ...
        if (!existingCart) {
            ...
            let existingItem = existingCart.items.find(item => item.comestic_id.equals(cartRequest.comestic_id));
    ...
    async update(currentUserId, cartRequest) {
        ...
        let updatedItem = existingCart.items.find(item => item.comestic_id.equals(cartRequest.comestic_id));
    ...
    async delete(currentUserId, cartToDelete){
        ...
        let deletedItem = existingCart.items.find(item => item.comestic_id.equals(cartToDelete.comestic_id));

        if(!deletedItem)
            throw Object.assign(new Error("Không tìm thấy sản phẩm này trong giỏ hàngg"), { status: 404 });
