...
class Medical_RecordService {
  ...
  async getByCustomerId(_id) {
    return await _repository.medical_RecordRepository.getByCustomerId(_id);
  }
  ...
  async update(id, status) {
    ...
    if (!medical_recordSelected)
      throw Object.assign(new Error("Không tìm thấy hồ sơ bệnh án này"), {
        status: 404,
      });

    medical_recordSelected.isHdden = status;
    await _repository.medical_RecordRepository.update(
      id,
      medical_recordSelected
    );
  ...
  async export_record(medical_record_id, res) {
    const recordToExport = await _repository.medical_RecordRepository.getById(
      medical_record_id
    );
    if (!recordToExport)
      throw Object.assign(new Error("Không tìm thấy hồ sơ bệnh án này"), {
    ...
    doc
      .fontSize(18)
      .text(`Medical Record: ${recordToExport.id}`, { align: "center" });
    ...
    if (recordToExport.prescription && recordToExport.prescription.length > 0) {
      ...
      const [comestics, medicines, treatments] = await Promise.all([
        comesticIds.length
          ? _repository.comesticRepository.find(comesticIds)
          : [],
        medicineIds.length
          ? _repository.medicineRepository.find(medicineIds)
          : [],
        treatmentIds.length
          ? _repository.treatmentRepository.find(treatmentIds)
          : [],
      ]);

      const comesticMap = Object.fromEntries(
        comestics.map((c) => [c._id.toString(), c.name])
      );
      const medicineMap = Object.fromEntries(
        medicines.map((m) => [m._id.toString(), m.name])
      );
      const treatmentMap = Object.fromEntries(
        treatments.map((t) => [t._id.toString(), t.name])
      );
