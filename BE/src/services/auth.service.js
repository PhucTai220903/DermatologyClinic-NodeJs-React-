...
exports.register = async (newUserRequest) => {
    ...
    if (!newUserRequest.name || newUserRequest.name.trim().length < 2) {
        throw Object.assign(new Error("Tên phải có ít nhất 2 ký tự"), { status: 400 });
    }

    // Validate age
    if (!newUserRequest.age || newUserRequest.age < 0 || newUserRequest.age > 120) {
    ...
    // Validate role
    const validRoles = ["customer", "admin", "doctor", "pharmacist"];
    if (!validRoles.includes(newUserRequest.role)) {
        throw Object.assign(new Error("Vai trò không hợp lệ"), { status: 400 });
    }
...
exports.login = async (name, password) => {
    ...
    if (name.length < 2) {
        throw Object.assign(new Error("Tên đăng nhập không hợp lệ"), { status: 400 });
    }
    ...
    if (!user) {
        throw Object.assign(new Error("Tên người dùng không tồn tại"), { status: 404 });
    }
    ...
    if (!isMatch) {
        throw Object.assign(new Error("Sai mật khẩu. Vui lòng đăng nhập lại"), { status: 401 });
    }
