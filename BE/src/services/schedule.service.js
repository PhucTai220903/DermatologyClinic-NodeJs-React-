...
class ScheduleService {
    ...
    async update(entity) {
    const existingSchedule = await _repository.scheduleRepository.getById(entity.schedule_id);
    if (!existingSchedule) {
        throw Object.assign(new Error("Không tìm thấy lịch trình này"));
    }

    const scheduleToUpdate = existingSchedule.schedules.find(
        schedule => schedule.schedule_date.toISOString().split("T")[0] === entity.schedule_date
    );
    ...
    await _repository.scheduleRepository.update(entity.schedule_id, existingSchedule);
